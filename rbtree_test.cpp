 ...
 *       이중 연결 리스트로 구현
 *       Red / Black은 식별하기 쉽게 enum이용 했으며, bool 이용시 데이터 크기
 * 절약
 *
 *       class RBTree
 *
 ...
 *                            leaf node라는 것만 알면 되기 때문에 새로운 노드
 * 삽입 때마다 leaf node를 생성 해줄 필요없이 모든 말단 노드들은 이 leaf node를
 * 가리키는 식으로 구현 leaf node는 항상 black
 *
 ...
 *               Transplant => 삭제 시 이용하며, 삭제할 노드의 자식 노드를
 * 부모노드에 연결해주는 함수 RotateRight(x) => x기준 오른쪽으로 회전
 *               RotateLeft(x) => x기준 왼쪽으로 회전
 *
 *               Inorder,Preorder,Postorder => 순회 함수
 *               tree_minimum(x), tree_maximum(x) => 노드 x 기준으로 가장 왼쪽,
 * 오른쪽 return 함수
 *
 *               DisplayMenu, SelectMenu => 초기 Menu판 print 함수
 *               Insert_helper,Delete_helper,order_helper,print_helper => 각각
 * 수행시 입력받고 조건 에러 처리 위한 함수 와 tree print 해주는 함수
 *
 *       InsertFixUp과 DeleteFixUp에서 각 case에 대한 설명은 github에 적어
 * 놓았다.
 *
 * 작성자 : gowoonsori
 ...
 private:
  ...
  void InsertFixUp(NodePtr z) {
    ...
    while (z != root && z->parent->color == RED) {
      ...
      NodePtr uncle = (z->parent == grandparent->left) ? grandparent->right
                                                       : grandparent->left;
      bool side =
          (z->parent == grandparent->left)
              ? true
              : false;  // if p[z]가 p[p[z]]의 왼쪽 자식이면 1 / 오른쪽이면 0
