...
class vector {
 private:
  ...
  void shift_elem_back(typename _Allocator::size_type pos,
                       typename _Allocator::size_type n) {
    if (_size == 0) return;
    for (typename _Allocator::size_type i = 0; i < _size - pos; ++i) {
      _allocator.construct(data + n + _size - i - 1, data[_size - i - 1]);
      _allocator.destroy(data + _size - i - 1);
    }
  }
  void shift_elem_front(typename _Allocator::size_type pos, int n) {
    for (size_type i = pos; i < _size - n ; i++)
	{
		_allocator.construct(data + i, data[i + n]);
		_allocator.destroy(data + i + n);
	}
  }
 ...
 public:
  ...
  explicit vector(size_type n, const value_type &val = value_type(),
      ...
      : data(NULL), _capacity(n), _size(n), _allocator(alloc) {
    data = _allocator.allocate(n);
    for (size_type i = 0; i < n; ++i) _allocator.construct(data + i, val);
  };
  /* range */
  ...
  vector(InputIterator first, InputIterator last,
         ...
         typename ft::enable_if<
             !ft::is_integral< InputIterator >::value >::type * = NULL)
      : data(NULL), _allocator(alloc) {
    int n = 0;
  ...
  void resize(size_type n, value_type val = value_type()) {
    if (_capacity < n)
      ...
      for (int i = 1; n + i < _size; ++i) _allocator.destroy(&data[i]);
  };
  ...
  void push_back(T s) {
    if (_capacity <= _size) expandCapacity();
    _allocator.construct(data + _size, s);
    _size++;
  }

  void pop_back() {
    if (_size > 0) _allocator.destroy(data + --_size);
  }
